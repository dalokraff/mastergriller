...
--                         for i=1, #mod.tisch do
--                             if item_one.skin == mod.tisch[i].swap_skin and (WeaponSkins.skins[mod.tisch[i].swap_skin][mod.tisch[i].swap_hand] == mod.tisch[i].path) then
--                                 local sound = sounds[event]
--                                 -- mod:echo(event)
--                                 if sound then
--                                     event = sound
--                                 end
...
-- mod:hook_safe(DamageUtils, "server_apply_hit", function (t, attacker_unit, target_unit, hit_zone_name, hit_position, attack_direction, hit_ragdoll_actor, damage_source, power_level, damage_profile, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger, blocking, shield_breaking_hit, backstab_multiplier, first_hit, total_hits, source_attacker_unit)
--     if mod.PAN_UNIT[damage_source] then
--         if mod.PAN_UNIT[damage_source].unit_name == 'units/gold_pan' then
--             local world = Managers.world:world("level_world")
--             local wwise_world = Wwise.wwise_world(world)
--             local event = "pan_melee_new01"
--             local sound = WwiseWorld.trigger_event(wwise_world, event, mod.PAN_UNIT[damage_source].unit)
--         end
--     end
-- end)
...
--     -- local unit_name = Unit.get_data(unit_3p, 'unit_name')
...
--     local unit_type = data_melee.item_data.key
--     local unit = data_melee['right_unit_1p']
--     local unit_name = data_melee['right_hand_unit_name']
--     mod.PAN_UNIT[unit_type] = nil
--     if unit_name == 'units/gold_pan' then 
--         mod.PAN_UNIT[unit_type] = {
--             unit = unit,
--             unit_name = unit_name,
...
-- mod:hook_safe(EffectHelper, "play_surface_material_effects", function (effect_name, world, hit_unit, position, rotation, normal, sound_character, husk, unit, hit_actor)
--     -- mod:echo(hit_unit)
-- end)
...
mod:hook_safe(ActionSweep,'_play_environmental_effect' ,function (self, weapon_rotation, current_action, hit_unit, hit_position, hit_normal, hit_actor)
    mod:echo(self.weapon_unit)
    mod:echo(Unit.get_data(self.weapon_unit, 'unit_name'))
